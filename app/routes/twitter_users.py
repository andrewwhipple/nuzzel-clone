...
def create_twitter_user_from_id(
    ...
    db_twitter_user = twitter.create_twitter_user(
...
def read_tweets_by_twitter_user_id(
    twitter_user_id: str, db: dependencies.Session = Depends(dependencies.get_db)
):
    twitter_user = crud.get_twitter_user(db, twitter_user_id=twitter_user_id)
    if twitter_user is None:
...
def read_tweets_of_following_by_twitter_user_id(
    ...
    if twitter_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    tweets = crud.get_following_tweets(
...
def get_following_by_user_id(
    twitter_user_id: str,
    db: dependencies.Session = Depends(dependencies.get_db),
    max_results: dependencies.Optional[int] = 1000,
):
    following = dependencies.client.get_users_following(
        id=twitter_user_id,
        user_auth=True,
    ...
    if following.data:
        for following_user in following.data:
            twitter_user = crud.get_twitter_user(
                db=db, twitter_user_id=following_user.id
            )
            if not twitter_user:
                ...
                follows.create_follows(
                    dependencies.schemas.FollowCreate(
...
def create_tweets_of_a_twitter_user_by_id(
    ...
    if len(twitter_user.tweets) > 0:
        ...
        tweets_array = dependencies.client.get_users_tweets(
            id=twitter_user_id,
            user_auth=True,
            max_results=10,
        ...
        for tweet in tweets_array.data:
            ...
            if tweet.entities:
                ...
                if "urls" in entities_data:
                    ...
                    tweet_create = dependencies.schemas.Tweet(
                        id=tweet.id,
                        url=tweet_url,
                        text=tweet.text,
                        twitter_user_id=twitter_user_id,
